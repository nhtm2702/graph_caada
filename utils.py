...
def balanced_binary_loss(logits,labels):
    ...
    if not losses:
        raise ValueError("No valid binary labels were provided.")

    return torch.stack(losses).mean()


def binary_entropy(probabilities,eps=1e-8):

    probabilities = probabilities.clamp(min=eps,max=1.0 - eps)

    entropy = -(probabilities * torch.log(probabilities)+ (1.0 - probabilities)* torch.log(1.0 - probabilities))
